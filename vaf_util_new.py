...
def get_crops_landmarks(face_detector, sp68, img, roi_delta=0.0, min_score=0.0):
    ...
    num_faces = len(dets)
    if num_faces == 0:
        return face_crops, final_landmarks, crop_coords
    ...
    for num in range(len(dets)):
        ...
        # exact_rect = dlib.rectangle(extended_roi_left, extended_roi_top,
        #                           extended_roi_right, extended_roi_bottom)
        # crop_coords.append(exact_rect)
        crop_coords.append((extended_roi_left, extended_roi_top, 
                           extended_roi_right, extended_roi_bottom))
